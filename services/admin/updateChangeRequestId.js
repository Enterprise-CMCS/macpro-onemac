...
export async function main(event) {
  ...
  const results = await dynamoDb
    .query({
      ...
      ProjectionExpression: "transmittalNumber,userId,id",
  ...
  } else if (results.Items[0].transmittalNumber != fromTransmittalNumber) {
    throw new Error(
      "Incorrect Transmittal number found - can not do auto update"
    );
  }

  //update the transmittalNumber to the input transmittalNumber
  await dynamoDb
    .update({
      TableName: process.env.tableName,
      Key: { userId: results.Items[0].userId, id: results.Items[0].id },
      UpdateExpression: "SET transmitttalNumber = :toTransmittalNumber",
      ExpressionAttributeValues: {
